...
if [ -f /app/venv/.deps_installed ]; then
  ...
  if [ -n "$_agent_src" ]; then
    ...
    # Stage into a PERSISTENT dir (/app is part of the image layer, survives
    # container restarts). The editable install references this path via the
    # __editable__ finder, so it must NOT be removed after install — otherwise
    # "AIAgent not available" returns on next start. (Patched: upstream used
    # /tmp + rm -rf, which broke editable imports.)
    _stage_src="/app/hermes-agent-build"
    rm -rf "$_stage_src"
    mkdir -p "$_stage_src"
    ...
    # KEEP the staged source — the editable finder resolves imports through it.
    # (Upstream removed it with `rm -rf`, which broke editable imports.)
  else
    echo ""
